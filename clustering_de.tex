...
\section{Clustering in statischen Graphen}
  ...
  \subsection{Modularit"atsoptimierung}
    ...
    Partitionierung $C(\Omega)$ eines Graphen $\Omega$ einem guten Clustering entspricht. Eine optimale Paritionierung entspricht
    somit dem Clustering des Graphen.
    ...
    Eine solche optimale Partitionierung, und somit ein Clustering kann mithilfe der Heuristik in [] approximiert werden. Der wesentliche Bestandteil
    ...
    wird effizient berechnet werden kann:
    \begin{align}
    ...
    \begin{enumerate}
      \item Am Anfang wird f"ur jeden Knoten ein einzelner Cluster erstellt
      \item berechnung wiederholt alle Knoten, verschieben von i zu den nachbar clustern, nur falls delta q > 0 
      \item Falls lokales maxima erreicht erstellen eines neuen graphen, cluster werden zu knoten gewichte zwischen knoten eines cluster werden zu selfloops
	    gewichte zwischen clustern zu gewichte zwischen knoten
      \item beginne bei schritt 2 bis modularit"at nicht mehr erh"oht werden kann
    \end{enumerate}
