...
\maketitle
  ...
  Traditionally, these networks consisted of relations between various
  members of a computer network, but nowadays, graphs are used for a
  wide variety of applications, from biological networks[?] over
  control methods for multi agent systems[?], to the communication patterns
  in the world wide web. viele real world networks haben deutlich anderen
  eigenschaften wie die klassischen random distributed graphs (ra- euzen 
  name :D). diese eigenschaften sind hoher clustering coefficent, groesse,
  etc.
